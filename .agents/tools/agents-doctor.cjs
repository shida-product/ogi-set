...
for (const p of files) {
  ...
  if (budget) {
    ...
    if (over.length)
      red.push(`${rel}: 予算超過（${over.join(" / ")}）→ 古い分を changelog/plan へ移送`);
  }
}
...
if (locksRaw) {
  ...
  if (rows.length) {
    yellow.push(
      `state/locks.md: アクティブ lock ${rows.length}件が残存 → 終了済みなら自分の行を削除（stale は30分で他セッションが掃除可）`
    );
  }
}
...
if (brokenCount) {
  yellow.push(
    `リンク切れ ${brokenCount}件（.agents 内の相対リンク）:\n    - ` +
      brokenSample.join("\n    - ") +
