...
function setupMasterSheet() {
  ...
  // 初期データ（システム内のDEFAULTSと同等）
  var defaultProducts = [
    ['shinal', 'シナール', 1900, '主剤（C・システイン）'],
    ['haichi', 'ハイチオール', 2100, '主剤（C・システイン）'],
    ['haishi', 'ハイシー顆粒', 1900, '主剤（C・システイン）'],
    ['trane500', 'トラネ500mg「YD」', 3900, '肝斑（トラネキサム）系'],
    ['trane250', 'トラネ250mg「YD」', 2500, '肝斑（トラネキサム）系'],
    ['transa250', 'トランサミン250mg', 2700, '肝斑（トラネキサム）系'],
    ['transa500', 'トランサミン500mg', 4700, '肝斑（トラネキサム）系'],
    ['tocoNico', 'トコフェロール200mg', 2200, 'ビタミンE系'],
    ['yubera50', 'ユベラ50mg', 2000, 'ビタミンE系'],
    ['yuberaN100', 'ユベラN100mg', 2200, 'ビタミンE系'],
    ['yuberaN200', 'ユベラN200mg', 2700, 'ビタミンE系'],
    ['noivitan', 'ノイロビタン配合錠', 2200, 'その他']
  ];
  ...
  // 1. 各テーブルのヘッダー書き込み
  sheet.getRange('A1:D1').setValues([['商品ID', '商品名', '単品価格', 'グループ']]);
  ...
  if (defaultProducts.length > 0) {
    sheet.getRange(2, 1, defaultProducts.length, 4).setValues(defaultProducts);
  }
  ...
  var headerRanges = ['A1:D1', 'F1:H1', 'J1:M1', 'O1:P1'];
  ...
  if (lastRow > 0) {
    sheet.getRange(1, 1, lastRow, 16).setBorder(true, true, true, true, true, true, '#D0D3D4', SpreadsheetApp.BorderStyle.SOLID);
  }
  ...
  ui.alert('初期設定完了', '「マスタ」シートの生成と初期データの配置が完了しました！', ui.ButtonSet.OK);
}
